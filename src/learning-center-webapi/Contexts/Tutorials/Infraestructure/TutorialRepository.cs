...
public class TutorialRepository(LearningCenterContext context) : BaseRepository<Tutorial>(context), ITutorialRepository
{
    ...
    {
        var result = await _context.Set<Tutorial>().Include(t => t.Chapters).ToListAsync();
        return result;
    }

    public async Task<Tutorial?> GetTutorialWithChaptersAsync(int id)
    {
        var result = await _context.Set<Tutorial>().Include(t => t.Chapters).FirstOrDefaultAsync(t => t.Id == id);
        return result;
    }

    public async Task<Tutorial?> GetTutoriaByTitleAsync(string title)
    {
        var result = await _context.Set<Tutorial>().FirstOrDefaultAsync(t => t.Title == title);
        return result;
    }
}